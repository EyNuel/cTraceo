...
void    cValues2D(uintptr_t, uintptr_t, float*, float*, float**, float, float, float*, float*, float*, float*, float*, float*);

void    cValues2D(uintptr_t nx, uintptr_t ny, float* xTable, float* yTable, float** cTable, float xi, float yi, float* ci, float* cxi, float* cyi, float* cxxi, float* cyyi, float* cxyi){
    ...
    float**    tempfloat2D = mallocFloat2D(3,3);
    ...
    for(a=0; a<3; a++){
        for(b=0; b<3; b++){
            DEBUG(8, "tempfloat2D[%u][%u] = cTable[%u][%u] = ", (uint32_t)a, (uint32_t)b, (uint32_t)(j+a), (uint32_t)(i+b));
            tempfloat2D[a][b] = cTable[j+a][i+b];
            DEBUG(8, "%lf.\n", cTable[j+a][i+b]);
        }
    }
    
    intBarycParab2D( &xTable[i], &yTable[j], tempfloat2D, xi, yi, ci, cxi, cyi, cxxi, cyyi, cxyi);
    freeFloat2D(tempfloat2D,3);
    DEBUG(8, "out, ci: %lf\n", *ci);
}
